...
void Adc_Init(const Adc_ConfigType* ConfigPtr)
{
    ...
    // Select ADC instance
    ADC_TypeDef* adcInstance = (ADC_TypeDef*)ConfigPtr->Instance;

    // Configure all channel GPIOs 
    for (uint8 i = 0; i < ConfigPtr->numChannels; i++) {
        uint8 channel = ConfigPtr->Channels[i];   
    ...
    for (uint8 i = 0; i < ConfigPtr->numChannels; i++) {
        if (adcInstance == ADC1) {
            // For ADC1, channel IDs are 0-15
            ADC_RegularChannelConfig(adcInstance, ConfigPtr->Channels[i].ChannelId, ConfigPtr->Channels[i].Rank, ConfigPtr->Channels[i].SamplingTime);
        }
        else if (adcInstance == ADC2) {
            // For ADC2, channel IDs are 16-31
            ADC_RegularChannelConfig(adcInstance, ConfigPtr->Channels[i].ChannelId - 16, ConfigPtr->Channels[i].Rank, ConfigPtr->Channels[i].SamplingTime);
        }
        else {
            // Unsupported ADC instance
            return;
        }
    }
...
void Adc_StartGroupConversion(Adc_GroupType Group){
    ...
    ADC_TypeDef* adcInstance = (ADC_TypeDef*)Adc_Groups[Group].Instance;
...
void Adc_EnableHardwareTrigger(Adc_GroupType Group)
{
    ...
    adcInstance->CR2 &= ~(0xE0000);  // Clear bits 17:19
    ...
    adcInstance->CR2 |= ADC_ExternalTrigConv_T1_CC1;

    // Step 3: Enable external trigger by setting EXTTRIG (bit 20)
    adcInstance->CR2 |= (1 << 20);  // EXTTRIG bit
